...
public abstract class JsonGeneratorTask<T extends PersistableConfigurationObject> extends GeneratorTask<T> {

    public JsonGeneratorTask() {
        generator = new PersistableConfigurationObjectGenerator<T>() {
            @Override
            public T create() {
                return JsonGeneratorTask.this.create();
            }

            @Override
            public void configure(T object) {
                JsonGeneratorTask.this.configure(object);
            }
        };
    }
